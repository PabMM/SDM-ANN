# %% Regression ANN + NSA
# P. Manrique. March 2024 - From P. Diaz TCASI-ANN-SDM
...
for file in datasetfiles:
    ...
    # saving datasets for cross-validation
    val_path = '../VALIDATION/DATASET/'
    df_val = np.concatenate((x_test,y_test),axis=1)
    df_val = pd.DataFrame(df_val)
    df_val.columns = columns_names
    df_val = df_val.sample(n=min(1000,len(df_val)), replace=False, random_state=1)
    df_val.to_csv(val_path+model_name+'_Validation.csv', index=False)
